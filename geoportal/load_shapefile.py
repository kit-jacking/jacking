...
from geoportal.algorithms.utils import get_linestring_geometry
...
def create_graph_and_geodataframe(file_to_load: str, crs: str = "epsg:4326",
                                  directions_column_name: str = "KIERUNKOWOSC",
                                  road_class_column_name: str = "KLASADROGI") -> tuple[Graph, gpd.GeoDataFrame]:
    ...
    for i, row in df.iterrows():
        ...
        node_start = conjunction_manager.get_conjunction(probable_node_start, create_node_name_from_coords)
        node_end = conjunction_manager.get_conjunction(probable_node_end, create_node_name_from_coords)
        ...
        try:
            road_class = row[road_class_column_name]
        except:
            road_class = "autostrada"

        if direction == 0:
            edge = create_edge_from_linestring(geom, node_start, node_end, i, road_class)
            edge2 = create_edge_from_linestring(geom, node_end, node_start, i, road_class)
            edges.append(edge)
            edges.append(edge2)
        elif direction == 1:
            edge = create_edge_from_linestring(geom, node_start, node_end, i, road_class)
